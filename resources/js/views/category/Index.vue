...
export default {
    ...
    data() {
        return {
            ...
            message:'',
            activityLog: [
                {title: 'Total Categories', amount: 50, icon: 'mdi-account', color: 'cyan lighten-3'},
                {title: 'Total Subcategories', amount: 3433, icon: 'mdi-account-group-outline', color: 'green darken-2'},
    ...
    methods: {
        ...
        async datatableSearch($e){
            if ($e.length >= 3){
                ...
                await axios.get(`/api/user/search-user/${$e}?page=${this.page}`, {params:{'per_page':this.per_page},headers: { 'Authorization': 'Bearer ' + token }})
                    .then((response)=>{
            ...
            if ($e.length <= 0){
                let token = JSON.parse(window.localStorage.getItem('token'))
                await axios.get(`/api/user?page=${this.page}`, {params:{'per_page':this.per_page},headers: { 'Authorization': 'Bearer ' + token }})
                    .then((response)=>{
                        if (response.data.status != 200){
                            console.log(response.data.status)
                        }else {
