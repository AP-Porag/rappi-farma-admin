...
export default {
    ...
    methods:{
        ...
        async getItemData(){
            ...
            await axios.get(`/api/brand/${this.$route.params.id}`, {headers: { 'Authorization': 'Bearer ' + token }})
                .then((response)=>{
        ...
        async submit(){
            ...
            if (this.valid){
                ...
                let token = JSON.parse(window.localStorage.getItem('token'))
                await axios.put(`/api/brand/${this.$route.params.id}`,this.form_data, {headers: { 'Authorization': 'Bearer ' + token }})
                    .then((response)=>{
                        if (response.data.status != 200){
                            this.message = response.data.message;
                            this.error = true;
